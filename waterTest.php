...
if (isset($_POST['submit'])) {
    ...
    water_review($_SESSION['username'],$_SESSION['examId'],implode(',', $counter),implode(',', $product1_ans),'','');
...
if (isset($_POST['next'])) {
    ...
    $product1_ans = array();
    $ans_cnt = 0;
    $examNum = examId($_SESSION['username'], "water");
    while ($row = mysqli_fetch_assoc($examNum)) {
        if ($row['num'] != null) {
            $_SESSION['examId'] = "water_" . ($row['num']+1);
            
        } else {
            $_SESSION['examId'] = "water_1";
        }
    }
}
