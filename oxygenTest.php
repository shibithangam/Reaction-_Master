...
if (isset($_POST['submit'])) {
    ...
    oxygen_review($_SESSION['username'],$_SESSION['examId'],implode(',', $counter),implode(',', $product1_ans),'','');
...
if (isset($_POST['next'])) {
    ...
    $product1_ans = array();
    $ans_cnt = 0;
    $examNum = examId($_SESSION['username'], "oxygen");
    while ($row = mysqli_fetch_assoc($examNum)) {
        if ($row['num'] != null) {
            $_SESSION['examId'] = "oxygen_" . ($row['num']+1);
            
        } else {
            $_SESSION['examId'] = "oxygen_1";
        }
    }
}
